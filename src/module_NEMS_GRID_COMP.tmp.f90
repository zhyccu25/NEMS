...
!
      CALL ESMF_ConfigGetAttribute(config = CF_NEMS                     &  !<-- The NEMS configure object
                                  ...
                                  ,rc     = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=317, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
...
!
        CALL ESMF_ConfigGetAttribute(config=CF_NEMS                     &
                                    ...
                                    ,label ='nhours_fcst1:'             &
                                    ,rc    =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=335, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
!
...
!
        CALL ESMF_TimeIntervalSet(timeinterval=RUNDURATION              &  !<-- The forecast length (s) (ESMF)
                                 ...
                                 ,rc          =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=350, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
        CALL ESMF_ClockSet(clock       = CLOCK_NEMS                     &  !<-- The NEMS Clock
                          ,runDuration = RUNDURATION                    &  !<-- The forecast length (s) (ESMF)
                          ,rc          = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=355, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
!
!-----------------------------------------------------------------------
...
!
      CALL ESMF_ConfigGetAttribute(config = CF_NEMS                     &  !<-- The NEMS configure object
                                  ...
                                  ,rc     = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=372, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
...
! 
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ...
                                    ,label  = TRIM(PELAB)               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=405, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
...
!
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ...
                                    ,label  = 'HH_INCREASE:'            &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=455, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ,value  = hh_start                  &
                                    ,label  = 'HH_START:'               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=461, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ,value  = hh_final                  &
                                    ,label  = 'HH_FINAL:'               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=467, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
...
!
      DO I = 1, TOTAL_MEMBER
        EARTH_GRID_COMP(i) = ESMF_GridCompCreate (                      &
                            ...
                            ,rc           = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=492, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
...
!
      IF(ENS_SPS) THEN
        ENS_CPL_COMP=ESMF_CplCompCreate(name = "ENS Cpl component"      &  
                                       ,rc   = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=503, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END IF
!
!-----------------------------------------------------------------------
...
!
      DO I = 1, TOTAL_MEMBER
        CALL ESMF_GridCompSetServices(EARTH_GRID_COMP(I)                &  !<-- The EARTH gridded components
                                     ...
                                     ,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=520, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
!***  Register the Initialize, Run, and Finalize routines of
...
!
        CALL ESMF_CplCompSetServices(ENS_CPL_COMP                       &
                                    ...
                                    ,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=538, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
!
!-----------------------------------------------------------------------
...
!
      DO I = 1,TOTAL_MEMBER
        EARTH_IMP_STATE(I) = ESMF_StateCreate(                          &
                                        ...
                                        ,rc        = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=556, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
!
...
!
      DO I = 1,TOTAL_MEMBER
        EARTH_EXP_STATE(I) = ESMF_StateCreate(                                 &
                                        ...
                                        ,rc          = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=571, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
...
!
        ENS_CPL_IMP_STATE=ESMF_StateCreate(name   = "ENS_CPL_Import"         &
                                          ,stateintent = ESMF_STATEINTENT_IMPORT  &
                                          ,rc          = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=588, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
!
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
!
        ENS_CPL_EXP_STATE=ESMF_StateCreate(name = "ENS_CPL_Export"           &
                                          ,stateintent = ESMF_STATEINTENT_EXPORT  &
                                          ,rc        = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=600, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
...
! 
        DO I = 1, TOTAL_MEMBER
          IF(MEMBER_ID == I) THEN
            ...
            if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=615, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
...
!-----------------------------------------------------------------------
      ...
      CALL ESMF_ConfigGetAttribute(config = CF_NEMS &
                                   ...
                                   ,default = 0 &
                                   ,rc     = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=636, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      if (fhrot > 0) then
        CALL ESMF_TimeIntervalSet(restartOffset, h=fhrot,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=639, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        CURRTIME = STARTTIME + restartOffset
        call ESMF_ClockSet(CLOCK_NEMS, currTime=CURRTIME, rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=642, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      endif
!
!-----------------------------------------------------------------------
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompInitialize(gridcomp    = EARTH_GRID_COMP(I)  &
                                      ...
                                      ,userRc      = RC_USER             &
                                      ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=665, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=666, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        END IF
!
      END DO
!
...
!
        CALL ESMF_CplCompInitialize(cplcomp    =ENS_CPL_COMP            &
                                   ...
                                   ,userRc     =RC_USER                 &
                                   ,rc         =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=689, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=690, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
!
!-----------------------------------------------------------------------
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompRun(gridcomp    = EARTH_GRID_COMP(I)        &
                               ...
                               ,userRc      = RC_USER                   &
                               ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=766, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=767, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
        END IF
!
      END DO
!
...
!
          CALL ESMF_CplCompRun(cplcomp    =ENS_CPL_COMP                 &
                              ...
                              ,userRc     =RC_USER                      &
                              ,rc         =RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=794, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=795, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          CALL ESMF_VMBarrier(vm = VM_GLOBAL, rc = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=798, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
...
!
          CALL ESMF_ClockGet(clock       = CLOCK_NEMS                   &
                            ...
                            ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=812, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
...
!
          CALL ESMF_ClockSet(clock    = CLOCK_NEMS                      &
                            ,currTime = CURRTIME                        &
                            ,rc       = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=819, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
!
          CALL ESMF_TimeIntervalGet(timeInterval = RUNDURATION          &
                                   ,h            = HH                   &
                                   ,rc           = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=829, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          HH = HH + HH_INCREASE
!
          CALL ESMF_TimeIntervalSet(timeInterval = RUNDURATION          &
                                   ,h            = hh                   &
                                   ,rc           = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=836, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          CALL ESMF_ClockSet(clock       = CLOCK_NEMS                   &
                            ,runDuration = RUNDURATION                  &
                            ,rc = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=841, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
!***  Execute the Run step of each element in the EARTH component
!***  array.
...
!
            IF(MEMBER_ID == J) THEN
                CALL ESMF_GridCompRun(gridcomp    = EARTH_GRID_COMP(J)  &
                                     ...
                                     ,userRc      = RC_USER             &
                                     ,rc          = RC)
                if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=863, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
                if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=864, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
            END IF
!
          END DO
!
...
!
         CALL ESMF_ClockGet(clock       = CLOCK_NEMS                    &
                           ,runDuration = RUNDURATION                   &
                           ,rc          = RC)
         if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=879, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
!
      END IF sps
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompFinalize(gridcomp    = EARTH_GRID_COMP(I)   &
                                    ...
                                    ,userRc      = RC_USER              &
                                    ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=946, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=947, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
        END IF
!
      END DO
!
...
!
        CALL ESMF_CplCompFinalize(cplcomp    =ENS_CPL_COMP              &
                                 ...
                                 ,userRc     =RC_USER                   &
                                 ,rc         =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=968, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
        if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=969, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
!
      END IF
!
!-----------------------------------------------------------------------
